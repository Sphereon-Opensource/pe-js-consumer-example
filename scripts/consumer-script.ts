...
function checkChapiHttpEdu() {
  ...
  const result: EvaluationResults = pex.evaluatePresentation(
    ...
    vpSimple, [ProofType.BbsBlsSignatureProof2020]
  );
...
function checkChapiHttpEdu_selectFrom() {
  ...
  const result: SelectResults = pex.selectFrom(
    ...
    vpSimple.verifiableCredential,
    [wallet.holder],
    ['Ed25519Signature2018',ProofType.BbsBlsSignatureProof2020]
  );
  console.log("result: ", JSON.stringify(result, null, 2));
}
...
function checkDiwala() {
  ...
  pex = new PEX();
  const presentationDefinition: PresentationDefinitionV1 = getFileAsJson(
    "./resources/diwala/pd.json"
  );
